...
def pandas_group():
    ...
    print(df_cc_vel)
    print(df_cc_cog)
    csv_path =str(new_dir.rstrip('csv/'))+'/CC'+str(i)+'cog_vel.csv' #'SaveFolder/addData2.csv'/cc09-14-20-52/cogvel1.csv
    print(csv_path)
    df_cc_cog.to_csv(csv_path, index=True,header=True, float_format='%.4f' )

    df_cc_vel.to_csv(csv_path, index=True,header=True, float_format='%.4f' )
