...
def pandas_group():
    ...
    for i in range(kaisu):
        ...
        #print(df_cog.at['0','field.data0'])

        print(type(df_cog.iloc[0,0]))

        
        #print(df_cog.iloc['0']['%time'])
        df_cog_time_m = (df_cog_time - df_cog.iloc[0,0]).astype(float) * 0.000000001 ##unixtimeを見やすくするために無理やり小さく、そして0.1^9で小数点が無かったunixtimeを正確な秒へ
        df_vel_time_m = (df_vel_time - df_cog.iloc[0,0]).astype(float) * 0.000000001 ##unixtimeを見やすくするために無理やり小さく、そして0.1^9で小数点が無かったunixtimeを正確な秒へ
        ...
        csv_path = str(new_dir.rstrip('csv/'))+'/t_cog_vel'+str(i)+'.csv' #'SaveFolder/addData2.csv'/cc09-14-20-52/cogvel1.csv
